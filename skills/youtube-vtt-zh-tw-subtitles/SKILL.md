...
代理應在要存放 `vtt/` 與 `.tmp/` 的目前工作目錄執行此流程。
...
- 目標檔名仍必須是 `vtt/{videoId}.zh-TW.vtt`。
...
node --experimental-strip-types scripts/translate-vtt-with-agy.ts \
  --input vtt/{videoId}.en.vtt \
  --output vtt/{videoId}.zh-TW.vtt \
  --workdir .tmp/vtt-translation-{videoId} \
  --chunk-size 400 \
  --overlap 120 \
...
| 參數 | 用途 |
|---|---|
| `--input` | 來源 VTT。常見是 `vtt/{videoId}.en.vtt`。 |
| `--output` | 目標 VTT。必須使用 `vtt/{videoId}.zh-TW.vtt`。 |
| `--workdir` | 中間檔目錄。建議使用 `.tmp/vtt-translation-{videoId}`。 |
...
```text
.tmp/vtt-translation-{videoId}/responses/batch-*.tsv
.tmp/vtt-translation-{videoId}/source.tsv
```
...
const src = fs.readFileSync(`vtt/${videoId}.en.vtt`, 'utf8');
const out = fs.readFileSync(`vtt/${videoId}.zh-TW.vtt`, 'utf8');
...
- `.tmp/vtt-translation-{videoId}/validation.txt` 包含 `ok`。
...
- 翻譯腳本失敗且無法安全修正。
- 驗證未通過。
- 已成功產出並驗證 `vtt/{videoId}.zh-TW.vtt`。
